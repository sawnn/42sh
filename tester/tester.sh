...
{
  local testfn=".//.tester.$$"
  local runnerfn=".//.runner.$$"
  local refoutfn=".//.refer.$$"
  local shoutfn=".//.shell.$$"
...
load_test()
{
  ...
  prepare_test
  $WRAPPER
  nb=`$CAT .//.refer.$$ | $GREP -v '^_=' | $GREP -v '^\[1\]' | $WC -l`
  i=1
  ok=1
  while [ $i -le $nb ]
  do
    l=`$CAT .//.refer.$$ | $GREP -v '^_=' | $GREP -v '^\[1\]' | $HEAD -$i | $TAIL -1`
    a=`$CAT .//.shell.$$ | $GREP -v '^_=' | $GREP -v '^\[1\]' | $GREP -- "$l$" | $WC -l`
    if [ $a -eq 0 ]
    then
      ok=0
    fi
  ...
  then
    ...
    if [ $debug -ge 1 ]
    then
      echo "Test $id ($NAME) : KO - Check output in .//test.$$/$id/" 
      $MKDIR -p .//test.$$/$id 2>/dev/null
      $CP .//.shell.$$ .//test.$$/$id/mysh.out
      $CP .//.refer.$$ .//test.$$/$id/tcsh.out
    else
      echo "KO"
    fi
  fi
